...
fn test_xz_reader() {
    ...
    let data_xz = fs::read(&path_xz)
        .unwrap_or_else(|_| panic!("Failed to read test file: {}", path_xz));
    let data_gzip = fs::read(&path_gzip)
        .unwrap_or_else(|_| panic!("Failed to read test file: {}", path_gzip));
    let obj_xz = read_rds(&data_xz)
        .expect("Failed to parse xz compressed file").object;
    let obj_gzip = read_rds(&data_gzip)
        .expect("Failed to parse gzip compressed file").object;
